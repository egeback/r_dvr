def download_stream stream, output, force, title
...
def download_episode_svt_play options, url
  #Add tailing /
...
def download_episode_barn options, url
  #Add tailing /
  options[:folder]+="/" if options[:folder]!=nil && !(options[:folder][options[:folder].length-1] == '/')
...
def download_episodes options, url, search_string
